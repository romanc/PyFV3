...
from pyfv3.stencils import RemapProfile
...
class TranslateCS_Profile_2d(TranslateFortranData2Py):
    ...
    def compute(self, inputs):
        ...
        j1 = 0
        j2 = 0
        self.compute_func = RemapProfile(
            stencil_factory=self.stencil_factory,
            quantity_factory=self.grid.quantity_factory,
            kord=inputs["kord"],
            iv=inputs["iv"],
            dims=(I_DIM, J_DIM, K_DIM),  # [i1, i2, j1, j2],
        )
        self.make_storage_data_input_vars(inputs)
        backend = self.stencil_factory.backend
        if "qs" not in inputs:
            inputs["qs"] = utils.make_storage_from_shape(
                self.maxshape[0:2], backend=backend
            )
        else:
            qs_field = utils.make_storage_from_shape(
                self.maxshape[0:2], origin=(0, 0), backend=backend
            )
